...
[GitHub link for Project]<https://github.com/joseantonio11/Coursera-Practical-Machine-Learning> 
...
Based on the dataset the aim of this project is to predict the classe based on data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants.
...
## 5 - PLOTING THE DATA FOR PRELIMINARY EDA
...
Lets use this variable `classe` in the plot, to see the frequency of each levels in training dataset.

```{r}
plot(training$classe, col = "blue", main="Histogram of the Classe Levels")
```

Observe that A classe seems to be the most frequent and D classe the least.

## 6 - PARTITION TRAINING DATASET 

To enable cross-validation lets make two datas subsets using 70% for trainingSet and 30% for testSet.
This method will allow us to apply our ML Prediction Models and evaluate bias and variance.
...
Lets implement here the Regression Tree model and Random Forest.
...
The accuracy shows a good results using Rpart, but a great Regression Method, used for resolution of problems in Kaggle, is Random Forests.

Lets apply it to our Project. 

### 7.3 - Random Forest - Model choosed
...
Random Random Forest method outperformed the Decision Tree method.  
The accuracy for random forests was 0.9958 whereas the decision tree was 0.6953.

Since our accuracy is above 99% for the random forest model in our cross-validation data, we should see very few misclassifications as compared to the decision tree model, which will only be 69.5% accurate.
